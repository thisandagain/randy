...
Randy.prototype.listen = function (port, arg, callback) {
    ...
    async.auto({
        ...
        authenticate:   ['connect', function (callback) {
            if (arg.pass !== null) {
                async.parallel([
                    function (callback) {
                        self.store.auth(arg.pass, callback);
                    },
                    function (callback) {
                        self.pub.auth(arg.pass, callback);
                    },
                    function (callback) {
                        self.sub.auth(arg.pass, callback);
                    }
                ], callback);
            } else {
                callback(null);
        ...
        pubsub:         ['authenticate', function (callback) {
            self.sub.subscribe(self.ns);
            self.sub.on('message', self.listener);
            callback(null);
        }],

        sockets:        ['authenticate', function (callback) {
